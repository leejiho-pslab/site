...
- 진행(2026-08-19): jype 애드센스 반려(8/14, 가치 낮은 콘텐츠) — 원인 분석: 부스트가 목표 30 도달 후
  ...
  · starship 재검토 24일째 무소식 → 애드센스 커뮤니티에 문의 글 게시 완료(8/19, 운영자) — 답변 대기
- 비용 중지(2026-08-23): 인수인계 문서 확인 — API 크레딧 월 $20 한도 도달(9/1까지 정지)·자동충전 반복
  결제($11.22×8회)의 소비 주체가 이 레포 크론들. 조치: publish.yml 크론 생성 완전 중지(GENERATE 는
  수동 디스패치 명시 시에만)·content-boost 크론 비활성(수동만). 발행·빌드·배포·이미지는 무료로 계속.
  운영자 필수 조치: console.anthropic.com Billing 에서 Auto-recharge OFF. 재개 시점: 수익 발생 후
  또는 운영자 지시. 주의: 생성 중지로 4개 사이트 콘텐츠가 정체됨 — 심사(kkultip·jype 8/28 재신청)
  신선도와 트레이드오프. jype 재신청 전 보강은 naver-batch/boost 수동 실행으로 최소 비용 진행 가능.
